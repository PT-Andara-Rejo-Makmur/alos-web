...
type WorkspaceOption = { workspace: Membership["workspace"] };
...
export default function ItUsersPage() {
  ...
  function selectWorkspace(index: number, event: ChangeEvent<HTMLSelectElement>) {
    const option = workspaceOptions.find((item) => item.workspace.workspace_id === event.target.value);
    if (option) updateDraft(index, { workspace: option.workspace });
  }
