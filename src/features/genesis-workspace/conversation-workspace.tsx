...
import {
  ...
  type Confirmation,
} from "@/features/governance/control-ui";
import {
  type AgentRecord,
  type AgentRunResult,
  latestVersion,
...
} from "@/features/genesis-workspace/agent-designer";
import {
  supportsGenesisUpload,
  type GenesisUploadRecord,
  uploadExtractionLabel,
} from "@/features/genesis-workspace/uploads";
import {
  canManageDraftAgent,
  canTestActiveAgent,
  contextHref,
...
} from "@/features/genesis-workspace/types";
import {
  type Run,
...
export function GenesisChat({
  ...
  const initialWorkspace = activeWorkspaceId ?? "";
  ...
  const loadWorkspaceData = useCallback(async (nextWorkspaceId: string) => {
    const [conversationItems, agentItems, runItems] = await Promise.all([
      authenticatedApiRequest<GenesisConversation[]>(
        withQuery("/api/v1/genesis/conversations", { workspace_id: nextWorkspaceId }),
      ),
      authenticatedApiRequest<GenesisActiveAgent[]>(
        withQuery("/api/v1/genesis/active-agents", { workspace_id: nextWorkspaceId }),
      ),
      authenticatedApiRequest<Run[]>("/api/v1/workspaces/" + encodeURIComponent(nextWorkspaceId) + "/runs?limit=100"),
    ]);
    setConversations(conversationItems);
    setActiveAgents(agentItems);
    setRuns(runItems);
    try {
      const registry = await authenticatedApiRequest<AgentRecord[]>(
  ...
  const loadConversation = useCallback(
    async (
      ...
      const [messageItems, attached] = await Promise.all([
        authenticatedApiRequest<Message[]>(
          "/api/v1/genesis/conversations/" + encodeURIComponent(nextConversationId) + "/messages",
        ),
        authenticatedApiRequest<GenesisConversationContext[]>(
      ...
      const optionSets = await Promise.all(
        types.map((entityType) =>
          authenticatedApiRequest<GenesisContextOption[]>(
  ...
  useEffect(() => {
    ...
    void initialize();
  }, [activeWorkspaceId, loadConversation, loadWorkspaceData]);
  ...
  async function newConversation() {
    ...
    setError(null);
    try {
      const created = await authenticatedApiRequest<GenesisConversation>("/api/v1/genesis/conversations", {
        method: "POST",
        body: JSON.stringify({
  ...
  async function renameConversation(event: FormEvent) {
    ...
    setError(null);
    try {
      const updated = await authenticatedApiRequest<GenesisConversation>(
  ...
  async function loadContextOptions(entityType: ContextEntityType, search: string) {
    ...
    setError(null);
    try {
      const options = await authenticatedApiRequest<GenesisContextOption[]>(
        withQuery("/api/v1/genesis/context-options", {
          workspace_id: workspaceId,
          entity_type: entityType,
  ...
  async function attachContext(option: GenesisContextOption) {
    ...
    setError(null);
    try {
      const attached = await authenticatedApiRequest<GenesisConversationContext>(
  ...
  async function uploadGenesisFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const uploaded = await authenticatedApiRequest<GenesisUploadRecord>("/api/v1/genesis/uploads", {
        method: "POST",
        body: payload,
      });
      setUploads((current) => [
  ...
  async function promoteUpload(upload: GenesisUploadRecord) {
    ...
    setError(null);
    try {
      const document = await authenticatedApiRequest<UploadedDocument>(
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const result = await authenticatedApiRequest<TurnResult>(
      ...
      if (currentConversation?.title === "Percakapan baru") {
        await authenticatedApiRequest<GenesisConversation>(
  ...
  async function testAgent(agent: GenesisActiveAgent) {
    ...
    setError(null);
    try {
      const result = await authenticatedApiRequest<AgentRunResult>(
  ...
  async function deleteDraft(agent: AgentRecord) {
    ...
    setError(null);
    try {
      await authenticatedApiRequest("/api/v1/agents/" + encodeURIComponent(agent.agent_key) + "/draft", {
        method: "DELETE",
      });
      setAgentDrawerKey("");
      setConfirmation(null);
  ...
  return (
    <section className="alos-content genesis-workspace" aria-label="GENESIS Workspace">
      ...
      {showAgentRequest ? (
        <AgentRequestModal
          activeDivisionCode={activeDivisionCode}
          actor={actor}
      ...
      {agentResult ? (
        <div className="genesis-drawer-backdrop">
          <section aria-modal="true" className="genesis-picker-modal genesis-draft-result" role="dialog">
            ...
            <div><Link href="/genesis?view=agents">Open Agent Registry</Link><Link href="/genesis?view=agents&sub=releases">Open Governance</Link></div>
          </section>
        </div>
      ) : null}
    ...
    <div className="genesis-drawer-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section aria-modal="true" className="genesis-agent-drawer" role="dialog">
        ...
        <footer>
          ...
          <Link href="/genesis?view=agents&sub=releases">Open Governance</Link>
        </footer>
...
function AgentRequestModal({
  ...
  const [division, setDivision] = useState(activeDivisionCode ?? "");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const result = await authenticatedApiRequest<GenesisAgentDesignResponse>("/api/v1/genesis/agent-requests", {
        method: "POST",
        body: JSON.stringify({
          workspace_id: workspaceId,
