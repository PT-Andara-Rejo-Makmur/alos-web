  ...
  return <ProtectedDomainWorkspace activeNavKey="register-user" deniedTitle="User Management hanya untuk IT" divisionCodes={["IT", "TECHNOLOGY"]} workspaceKeys={["it", "technology"]} loadingLabel="Memverifikasi otoritas IT…">
    {({ actor }) => {
      ...
      function nextStep() {
        ...
        if (step === 2 && (!selectedWorkspaces.length || form.role_refs.length === 0)) { setMessage("Pilih minimal satu workspace dan satu role."); return; }
        setStep((current) => Math.min(3, current + 1));
      }
      function previousStep() { setMessage(""); setStep((current) => Math.max(1, current - 1)); }
      async function submit() {
        if (step !== 3 || !isAdmin || submitting || !selectedWorkspaces.length || form.role_refs.length === 0) return;
        ...
        } catch (error) { setMessage(apiMessage(error)); } finally { setSubmitting(false); }
      }
      if (!isAdmin) return <section className="panel workspace-panel"><h1>Akses Dibatasi</h1><p>Membership IT aktif dengan izin pengelolaan akun diperlukan.</p></section>;
      return <section className={styles.page}>
        <div className={styles.registrationHeader}><div><p className={styles.eyebrow}>IT · IDENTITY & ACCESS</p><h1 className={styles.title}>Register Akun Baru</h1><p className={styles.intro}>Buat akun baru dan tentukan akses awal pengguna.</p></div><Link className={styles.back} href="/workspace/it/users">Kembali ke Kelola Akun</Link></div>
