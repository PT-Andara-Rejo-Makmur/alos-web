...
export function ProjectPortfolioDashboard({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setFailed(false);
      try {
        setData(await authenticatedApiRequest<ProjectPortfolioSnapshot>(url, { signal: controller.signal }));
      } catch (error) {
        if ((error as Error).name !== "AbortError") setFailed(true);
      } finally {
  ...
  return (
    <>
      <ProjectCreatePanel activeWorkspace={activeWorkspace} onCreated={() => setReloadKey((value) => value + 1)} />
      <ProjectOperationsPanel onChanged={() => setReloadKey((value) => value + 1)} projects={data.projects} />
...
function ProjectOperationsPanel({ onChanged, projects }: { onChanged: () => void; projects: ProjectControl[] }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "update") {
        await authenticatedApiRequest(`/api/v1/projects/${selected.project_id}`, { method: "PATCH", body: JSON.stringify({ status: update.status, progress_percent: Number(update.progress_percent), deadline: update.deadline || null, budget_spent: update.budget_spent ? Number(update.budget_spent) : null }) });
        setNotice("Status dan progress proyek diperbarui.");
      } else if (mode === "milestone") {
        await authenticatedApiRequest(`/api/v1/projects/${selected.project_id}/milestones`, { method: "POST", body: JSON.stringify({ ...milestone }) });
        setMilestone({ title: "", due_date: "", status: "ON_TRACK" }); setNotice("Milestone berhasil ditambahkan.");
      } else {
        await authenticatedApiRequest("/api/v1/project-issues", { method: "POST", body: JSON.stringify({ workspace_id: selected.workspace_id, division_code: selected.division_code, project_id: selected.project_id, title: issue.title, description: issue.description, severity: issue.severity, due_date: issue.due_date || null }) });
  ...
  async function removeProject() {
    ...
    setSaving(true); setError(""); setNotice("");
    try {
      await authenticatedApiRequest(`/api/v1/projects/${selected.project_id}`, { method: "DELETE" });
      setProjectId("");
...
function ProjectCreatePanel({
  ...
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!activeWorkspaceProp.divisionCode) return;
    setSaving(true); setError("");
    try {
      await authenticatedApiRequest("/api/v1/projects", {
        method: "POST",
        body: JSON.stringify({
          workspace_id: activeWorkspaceProp.workspaceId,
          division_code: activeWorkspaceProp.divisionCode,
          code: form.code,
          name: form.name,
  ...
  return <section className="alos-content alos-project-create-shell"><div className="alos-operation-heading"><div><p className="alos-kicker">PORTFOLIO OPERATIONS</p><h3>Kelola proyek dalam scope Anda</h3></div><button className="alos-workspace-primary" onClick={() => setOpen((value) => !value)} type="button">{open ? "Tutup" : "+ Buat proyek"}</button></div>{error ? <div className="alos-operation-banner error">{error}</div> : null}{open ? <form className="alos-operation-form" onSubmit={(event) => void submit(event)}><label>Workspace<input disabled value={activeWorkspaceProp.workspaceLabel} /></label><label>Kode<input maxLength={40} minLength={2} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, "") })} required value={form.code} /></label><label>Kategori<input maxLength={80} minLength={2} onChange={(event) => setForm({ ...form, category: event.target.value })} required value={form.category} /></label><label className="wide">Nama proyek<input maxLength={160} minLength={2} onChange={(event) => setForm({ ...form, name: event.target.value })} required value={form.name} /></label><label>Tenggat<input onChange={(event) => setForm({ ...form, deadline: event.target.value })} type="date" value={form.deadline} /></label><button className="alos-workspace-primary" disabled={saving || !activeWorkspaceProp.divisionCode} type="submit">{saving ? "Menyimpan…" : "Simpan proyek"}</button></form> : null}</section>;
}
